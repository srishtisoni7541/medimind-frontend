...
import { Star, Clock, Building2, Heart, Users, Sparkles, Armchair, Stethoscope, Brain, MapPin, Phone, Award, Calendar, Ambulance, HelpCircle } from 'lucide-react';
import { useDispatch, useSelector } from 'react-redux';
import { useNavigate, useParams } from 'react-router-dom';
import { useSnackbar } from 'notistack';
import { fetchSingleHospital, clearHosError, clearHosMessage, updateHospitalReview } from "../../store/actions/hospitalaction";
...
function EdithosRating() {
  ...
  useEffect(() => {
    if (user && user.hosreviews && reviewId) {
      const reviewToEdit = user.hosreviews.find(review => review._id === reviewId);
      
      if (reviewToEdit) {
        // Map the review data to the new form structure
        setFormData({
          ratings: {
            staffFriendliness: reviewToEdit.staffFriendliness || 0,
            facilityClean: reviewToEdit.facilityClean || 0,
            waitingTime: reviewToEdit.waitingTime || 0,
            accessibility: reviewToEdit.accessibility || 0,
            appointmentEase: reviewToEdit.appointmentEase || 0,
            emergencyResponse: reviewToEdit.emergencyResponse || 0,
            overall: reviewToEdit.rating || 0
          },
          comment: reviewToEdit.comment || '',
          madeAppointment: reviewToEdit.appointmentEase > 0,
          usedEmergency: reviewToEdit.emergencyResponse > 0
        });
      } else {
  ...
  }, [user, reviewId, navigate, hospitalId, enqueueSnackbar]);
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      dispatch(updateHospitalReview(reviewData, reviewId, navigate));
    }
  };
   ...
   <div className="min-h-screen  bg-white py-8 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-gray-700 mb-2">병원 평가 수정하기</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl mx-auto">
          ...
          <div className="flex gap-4">
            <button
              type="button"
              onClick={() => navigate(`/${hospitalId}/hospitalreview`)}
